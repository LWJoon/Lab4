...
if(a==0 || b==0){
gtk_label_set_text(GTK_LABEL(message_label),"숫자를 입력하세요.");
return;
...
sprintf(resultMsg,"결과는 %d",result);
if(button==buttDiv){
double resultf=(double)a/b;
sprintf(resultMsg,"결과는 %.2lf",resultf);
...
label_A = gtk_label_new("첫번째 숫자");
label_B = gtk_label_new("두번째 숫자");
entry_A = gtk_entry_new();
entry_B = gtk_entry_new();
message_label = gtk_label_new("숫자를 입력하세요.");
